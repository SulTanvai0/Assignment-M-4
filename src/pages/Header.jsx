...
import { apiContext, newsDataContext } from "../context";
...
const Header = () => {
    ...
    const { setData } = useContext(newsDataContext);

    const rawData = useNewsQuery(api);

    useEffect(() => {
        if (api === 'http://localhost:8000/v2/top-headlines') {
            setData(rawData.articles)
        }
    }, [api, rawData.articles, setData]);


    function handelSetApi(category) {
        setApi(`http://localhost:8000/v2/top-headlines?category=${category}`);
        setData(rawData.articles)
    }
    ...
    const doSearch = useDebounce((term) => {
        ...
        if (term) {
            ...
            setApi(`http://localhost:8000/v2/search?q=${term}`)
            setData([...rawData.result])

        }
    }, 200);

    console.log(`%cThe console is showing whenever API is calling. API=${api}`, 'color: green; font-weight: bold;');
    ...
    return (
        <nav className="border-b border-black py-6 md:py-8">
            ...
            <div className="container mx-auto mt-6">
                ...
                >
                    {
                        HeaderData.map((data) => <li key={data.id} onClick={() => handelSetApi(data.category)}><a>{data.name}</a></li>)
                    }
